...
def features_to_zarr(phase):
    ...
    if phase == 'trainval':
        infiles = [
            'coco/trainval_36/trainval_resnet101_faster_rcnn_genome_36.tsv',
        ]
    elif phase == 'test':
        infiles = [
            'coco/test2015_36/test2015_resnet101_faster_rcnn_genome_36.tsv',
        ]
    else:
        raise SystemExit('Unrecognised phase')

    # Read the tsv and append to files
    boxes = zarr.open_group('coco/' + phase + '_boxes.zarr', mode='w')
    features = zarr.open_group('coco/' + phase + '.zarr', mode='w')
    image_size = {}
