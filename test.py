...
def get_logits(model, dataloader):
    ...
    attention = torch.FloatTensor(N, 10).zero_()
    bboxes = torch.FloatTensor(N, 10, 4).zero_()
    ...
    for v, b, q, a, i in iter(dataloader):
        ...
        att_topk, top_idx = torch.topk(att.squeeze(-1), 10, dim=-1, sorted=False)
        top_idx = top_idx.unsqueeze(-1).expand(batch_size, 10, 4)
        b_top = torch.gather(b, 1, index=top_idx)

        pred[idx:idx+batch_size,:].copy_(logits.data)
        attention[idx:idx+batch_size,:].copy_(att_topk.data)
        bboxes[idx:idx+batch_size,:,:].copy_(b_top.data)
    ...
    bar.update(idx)
    return pred, qIds, attention, bboxes


def make_json(logits, qIds, att, bboxes, dataloader):
    ...
    for i in range(logits.size(0)):
        ...
        result['attention'] = att[i].tolist()
        result['bboxes'] = bboxes[i].tolist()
...
if __name__ == '__main__':
    ...
    eval_dset = VQAFeatureDataset('test', dictionary, opt.dataroot, opt.img_root, opt.ratio, adaptive=False)
